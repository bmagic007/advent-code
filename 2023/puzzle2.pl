...
my $input = <<string_ending_delimiter;
...
my @good = ();
my $total = 0;
while(my($gameNum, $gamesArr) = each %data)
{
	my $passed = 1;
	my @games = @{$gamesArr};
	foreach(@games)
	{
		my %thisGame = %{$_};
		while(my($color, $maxVal) = each %allowedMax)
		{
			$passed = 0 if( $thisGame{$color}+0 > $maxVal );
		}
	}
	push (@good, $gameNum) if $passed;
	$total += $gameNum if $passed;
}
@good = sort {$a <=> $b} @good;
...
while(my($gameNum, $gamesArr) = each %data)
{
	...
	{
		my %thisGame = %{$_};
		while(my($color, $pullVal) = each %thisGame)
		{
			$maxes{$color} = 0 if(!$maxes{$color});
			$maxes{$color} = $pullVal if $maxes{$color} < $pullVal;
		}
	}
	# print "game $gameNum: ".Dumper(\%maxes);
	my $gamePower = 1;
	while(my($color, $maxVal) = each %maxes)
	{
		$gamePower = $gamePower * $maxVal if ($maxVal > 0);
	}
...
sub parseLine
{
	my $line = shift;
	my @sep = split(/:/, $line);
	my $gameNum = getGameNum(shift @sep);
	if($gameNum !=-1 && !$data{$gameNum})
	{
		my @t = ();
		$data{$gameNum} = \@t;
		my $pullsString = shift @sep;
		my @pulls = split(/;/, $pullsString);
		foreach(@pulls)
		{
			return 0 unless parsePull($gameNum, $_);
		}
	}
	else
	{
		print "Enountered redundant game or non-game: [$line]\n";
		return 0;
	}
	return 1;
}
...
sub parsePull
{
	...
	my $pull = shift;
	my @vals = split(/,/,$pull);
	my @games = @{$data{$gameNum}};
	my %gameBuild = ();
	foreach(@vals)
	{
		my $pair = getColorPair($_);
		return 0 unless $pair;
		$gameBuild{$pair->[1]} = $pair->[0];
		$colorPivot{$pair->[1]} = () if(!$colorPivot{$pair->[1]});
		$colorPivot{$pair->[1]}->{$pair->[0]} = () if(!$colorPivot{$pair->[1]}->{$pair->[0]});
		push(@{$colorPivot{$pair->[1]}->{$pair->[0]}}, $gameNum);
	}
	push (@games, \%gameBuild);
	$data{$gameNum} = \@games;
	return 1;
}
...
{
	my $string = shift;
	my @pair = ( $string =~ /\s*([^\s]*)\s+([^\s]*)$/g );
	return \@pair if($#pair = 1);
	return 0;
}
