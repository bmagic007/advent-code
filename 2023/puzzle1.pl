...
my @ar = split(/\n/, $input);
# my @ar = ('75sevennine14mzqljsjfbb7two');
my @numCollection = ();
my $loops = 0;
foreach(@ar)
{
	...
	my $in = lc $_;
	# print $in."\n";
	my @firstLast = getNums($in);
	# print Dumper(\@firstLast);
	# exit if $loops > 50;
	push (@numCollection, @firstLast[0].@firstLast[1]);
}
my $total = 0;
foreach(@numCollection)
{
	$total += $_ + 0;
}
print $total . "\n";
...
sub getNums
{
	...
	my %wordPos = %{getWordPos($in)};
	while(my($key, $value) = each %wordPos)
	{
		$posMap{$key} = $value;
	}
	# print Dumper(\%posMap);
	my @order = ();
	while(my($key, $value) = each %posMap)
	{
		push @order, $key;
	}
	@order = sort {$a <=> $b} @order;
	return ($posMap{@order[0]},$posMap{@order[$#order]}) if($posMap{@order[0]} && $posMap{@order[$#order]});
	return ('0','0');
}
...
sub getWordPos
{
	my $in = shift;
	my %poses = ();
	while(my($key, $value) = each %names_to_numbers)
	{
		my @s = split(/$key/, $in);
		my $pos = 0;
		for(my $i = 0; $i <= $#s; $i++)
		...
		if(substr($in,(length($key)*-1)) eq $key)
		{
			$poses{length($in) - length($key)} = $value;
			# print Dumper(\%poses);
		}
	}
	return \%poses;
